...
// "Rit" é o nosso incentivo (raiz de Ritmind) no lugar de "kudos" — ícone de
// corredor. Interno segue "kudos" (classe/rota/repo); só o rótulo muda.
function RunIcon({ size = 17 }: { size?: number }) {
  return (
...
function KudosBtn({ a, onToggle }: { a: SocialActivity; onToggle: () => void }) {
  return (
    <button className={`kudos${a.kudos_by_me ? " on" : ""}`} onClick={onToggle} aria-label="Mandar um Rit">
      <RunIcon />
