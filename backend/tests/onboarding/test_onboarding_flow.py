...
def test_strava_yes_sends_connect_link_and_skips_pace(tmp_path):
    ...
    conversation = [
        ...
        # sem ASK_PACE: cai direto em ASK_DAYS
        (
    ...
    # pergunta seguinte já é a de experiência (e depois dias)
    assert "já corre hoje" in strava_reply
    assert "Quais dias" in replies[4]
    ...
    # sem ASK_PACE, não há pace autodeclarado
    assert data["initial_pace_min_km"] is None
